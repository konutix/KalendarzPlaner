...
public class TimeInfiniteScroll : MonoBehaviour, IBeginDragHandler, IDragHandler, IScrollHandler
{
    ...
    /// </summary>
    [SerializeField]
    private InfiniteScrollContent scrollContent;
    ...
    private void HandleVerticalScroll()
    {
        ...
        int selectedIndex = positiveDrag ? scrollRect.content.childCount - 2 : 1;

        for (int i = 0; i < scrollRect.content.childCount; i++)
        {
            var text = scrollRect.content.GetChild(i).GetComponent<Text>();
            if (text)
            {
                text.color = (i == selectedIndex) ? Color.cyan : Color.black;
            }
        }
    }
